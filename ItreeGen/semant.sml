    ...
    let
      ...
      fun trExp (A.VarExp v) = trVar v
        ...
        | trExp (A.SeqExp ((e, _) :: es)) =
            ...
            in
              {exp=Tr.seqExp (#exp trE) (#exp trEs), ty=(#ty trEs)}
            end
                    ...
                    T.ARRAY (aty, u) =>
                      ...
                      in
                        (checkTyCompat T.INT (#ty trSz) pos;
                         ...
                         {exp=Tr.arrExp (#exp trSz) (#exp trInit) level,
                          ty=T.ARRAY (aty, u)})
                      end
              ...
              (case actualTy (#ty trV) pos of
                 T.ARRAY (aty, _) =>
                   (checkTyCompat T.INT (#ty trIdx) pos;
                    {exp=Tr.subsVar (#exp trV) (#exp trIdx) level,
                     ty=actualTy aty pos})
                ...
                (case S.look (env, name) of
                   ...
                   SOME (E.FUNentry {level=flevel, label, formals, result}) =>
                     let
                       ...
                       val trBdy = transExp env' tenv 0 flevel fdone body
                     in
