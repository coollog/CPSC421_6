...
fun transexp (env:E.env, tenv:E.tenv) expr =
  let fun g (A.NilExp) = {exp = Translate.intExp 0, ty=T.NIL}
        | g (A.IntExp i) = {exp = Translate.intExp i, ty = T.INT}
        | g (A.StringExp (_,_)) = {exp = Translate.intExp 0, ty=T.STRING}
        | g (A.AppExp {func,args,pos}) =
          {exp = Translate.intExp 0, ty=
        ...
        in
            case ty of
                ...
                (cmpField (symTyList, fields);
                 {exp = Translate.intExp 0,  ty=t})
              | x =>
                (error pos (if isSome x
                            then msgRecordExp02
                            else msgRecordExp01);
                 {exp = Translate.intExp 0,  ty=T.RECORD(map chkField fields, ref())}
                )
        end
        ...
        | g (A.BreakExp(pos)) =
          if (hd(!breakCnt)=0)
          then (error pos msgBreakExp; {exp = Translate.intExp 0, ty=T.UNIT})
          else {exp = Translate.intExp 0, ty=T.UNIT}
                ...
                case tenvEnt of
                    ...
                    in
                        case tyCmp(tenv,ty,#ty(initTy),pos) of
                            SOME(_) => {exp = Translate.intExp 0, ty=T.ARRAY(#ty(initTy),u)}
                          | NONE => (error pos msgArrayExp02; {exp = Translate.intExp 0, ty=T.ARRAY(T.INT,ref ())})
                    end
                  | SOME(t) => (error pos msgArrayExp03; {exp = Translate.intExp 0, ty=T.ARRAY(T.INT,ref())})
                  | NONE => (error pos msgArrayExp04; {exp = Translate.intExp 0, ty=T.ARRAY(T.INT,ref())})
            end
          )
          ...
          (
            ...
            in
                ...
                case expCmp(left,right,pos) of
                    SOME(t) => {exp = Translate.intExp 0, ty=T.INT}
                  | NONE => (error pos msgOpNeqExp02; {exp = Translate.intExp 0, ty=T.INT})
            end
          )
        | g (A.OpExp {left,oper=A.EqOp,right,pos}) =
          (
            ...
            in
                ...
                case expCmp(left,right,pos) of
                    SOME(t) => {exp = Translate.intExp 0, ty=T.INT}
                  | NONE => (error pos msgOpEqExp02; {exp = Translate.intExp 0, ty=T.INT})
            end
          )
        ...
        | g (A.OpExp {left,oper=A.GeOp,right,pos}) =
          (
            case expCmp(left,right,pos) of
                ...
                (
                  case t of
                      T.INT => {exp = Translate.intExp 0, ty=T.INT}
                    | T.STRING => {exp = Translate.intExp 0, ty=T.INT}
                    | _ => (error pos (msgCmpExp01^"\">=\"."); {exp = Translate.intExp 0, ty=T.INT})
                )
              | NONE => (error pos (msgCmpExp02^"\">=\"."); {exp = Translate.intExp 0, ty=T.INT})
          )
        ...
        | g (A.OpExp {left,oper,right,pos}) =
          (
            ...
            {exp = Translate.intExp 0,  ty=T.INT}
          )
          ...
          (
            ...
            in
                ...
                case tyCmp(tenv,t1,t2,pos) of
                    SOME(_) => {exp = Translate.intExp 0, ty=T.UNIT}
                  | NONE => (error pos msgAssignExp02; {exp = Translate.intExp 0, ty=T.UNIT})
            end
          )
                ...
                case envEnt of
                    ...
                    (
                      case nameFollow(tenv,ty,pos) of
                          SOME(t) => {exp = Translate.intExp 0, ty=t}
                        | NONE => (error pos msgH0x; {exp = Translate.intExp 0, ty=T.INT})
                    )
                  | SOME(E.FUNentry{level,label,formals,result}) => (error pos (msgH00^S.name(id)^")."); {exp = Translate.intExp 0, ty=T.INT})
                  | NONE => (error pos (msgH01^S.name(id)^msgH01b); {exp = Translate.intExp 0, ty=T.INT})
            end
          )
                ...
                case leftPiece of
                    ...
                    (
                      case isField(st,id) of
                          SOME(t) => {exp = Translate.intExp 0, ty=t}
                        | NONE => (error pos (msgH02^S.name(id)^")."); {exp = Translate.intExp 0, ty=T.INT})
                    )
                  | _ => (error pos msgH03; {exp = Translate.intExp 0, ty=T.INT})
            end
          )
          ...
          (
            ...
            in
                (
                  ...
                  case leftPiece of
                      SOME(T.ARRAY(t,_)) => {exp = Translate.intExp 0, ty=t}
                    | _ => (error pos msgH05; {exp = Translate.intExp 0, ty=T.INT})
                )
            end
          )
...
fun transprog prog =
  let
      val firstLv = Translate.newLevel {
              name = Temp.namedlabel "firstLv",
              parent = Translate.outermost,
              formals = []
          }
      val {exp, ty} = transexp (E.base_env, E.base_tenv) prog
  in
      Translate.procEntryExit {level = firstLv, body = exp};
      Translate.getResult ()
  end
