...
fun printtree (outstream, s0) =
  let fun say s =  TextIO.output (outstream, s)
      ...
      fun stm (T.SEQ (a, b), d) = (indent d; sayln "SEQ("; stm (a, d + 1);
        ...
        | stm (T.MOVE (a, b), d) = (indent d; sayln "MOVE("; exp (a, d + 1);
          sayln ","; exp (b, d + 1); say ")")
        | stm (T.EXP e, d) = (indent d; sayln "EXP("; exp (e, d + 1); say ")")
      and exp (T.BINOP (p, a, b), d) = (indent d; say "BINOP("; binop p;
          sayln ","; exp (a, d + 1); sayln ","; exp (b, d + 1); say ")")
        | exp (T.CVTOP (p, e, s1, s2), d) = (indent d; say "CVTOP[";
          say (makestring s1); say ","; say (makestring s2); say "]("; cvtop p;
          sayln ","; exp (e, d + 1); say ")")
        | exp (T.MEM (e, s), d) = (indent d; say "MEM["; say (makestring s);
          sayln "]("; exp(e,d+1); say ")")
        | exp (T.TEMP t, d) = (indent d; say "TEMP T"; say (makestring t))
        | exp (T.ESEQ (s, e), d) = (indent d; sayln "ESEQ("; stm(s, d + 1);
          sayln ","; exp (e, d + 1); say ")")
        | exp (T.NAME lab, d) = (indent d; say "NAME "; say (Symbol.name lab))
        | exp (T.CONST i, d) = (indent d; say "CONST "; say (makestring i))
        | exp (T.CONSTF r, d) = (indent d; say "CONSTF "; say (Real.toString r))
        | exp (T.CALL (e, el), d) = (indent d; sayln "CALL("; exp (e, d + 1);
          app (fn a => (sayln ","; exp (a, d + 2))) el; say ")")
